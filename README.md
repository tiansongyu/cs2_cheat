...
| **性能优化** | 多线程架构（数据/渲染分离）、实体缓存快慢路径、VSync 禁用、D3D9 低延迟渲染 |
...
本项目采用多项优化措施减少 ESP 拖框延迟：

| 优化项 | 说明 |
|--------|------|
| **多线程数据/渲染分离** | 数据读取线程全速运行，渲染线程独立绘制，互不阻塞 |
| **实体缓存快慢路径** | 地址解析+武器名等慢数据每 8 帧刷新，位置/骨骼等快数据每帧更新 |
| **Mutex 快照机制** | 渲染线程通过极短临界区拷贝数据快照，避免长时间持锁 |
| **禁用 VSync** | `SDL_HINT_RENDER_VSYNC=0` 消除垂直同步等待 |
| **D3D9 渲染后端** | 强制使用 Direct3D 9 后端，减少 DWM 合成延迟 |
| **窗口更新节流** | `SetWindowPos` 降频至 4Hz，避免每帧调用重量级 Win32 API |
