...
### 12.1 单次 IOCTL 成本拆解（Win11 23H2，i7-12700）
...
读 16 个 4 字节，`PsLookupProcessByProcessId` / `ObDereferenceObject` 摊到 16 次只算一次。每个 entry 的 `MmCopyVirtualMemory` ~500 ns + 验证 ~50 ns。
...
60 FPS = 16.6 ms / 帧。我们的数据线程独立于渲染线程，可以跑得更频繁。即便 200 Hz 数据采样也只需要 5 ms / 周期，IOCTL 总开销 ~12 µs 占用不到 0.3%。
